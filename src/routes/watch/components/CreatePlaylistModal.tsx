...
export function CreatePlaylistModal({
  ...
  const canSubmit = useMemo(() => {
    // Your backend currently requires both name + description
      ...
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-light-background text-black ring-1 ring-black/10 dark:bg-dark-background dark:text-white dark:ring-white/10">
        ...
        <div className="flex items-center justify-between border-b border-black/10 px-5 py-4 dark:border-white/10">
          ...
          <button
            ...
            onClick={onClose}
            disabled={creating}
            className="grid h-9 w-9 place-items-center rounded-xl bg-black/5 ring-1 ring-black/10 hover:bg-black/10 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white/5 dark:ring-white/10 dark:hover:bg-white/10"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-black/10 px-5 py-4 dark:border-white/10">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="inline-flex items-center gap-2 rounded-full bg-black px-4 py-2 text-xs font-semibold text-white hover:bg-black/85 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white dark:text-black dark:hover:bg-white/85"
          >
