...
export const Login = () => {
  ...
  const { login } = useAuth(); // ✅ use login from context (calls loginApi + revalidate internally)
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      await login(payload); // ✅ this calls loginApi then revalidate() — auth state updates correctly
      navigate("/");
    } catch (err: any) {
      console.log(err);
      toast.error(
